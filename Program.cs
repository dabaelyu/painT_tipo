...
namespace painT_qqqwww
{
    ...
    public class MainForm : Form
    {
        ...
        private void InitializeUI()
        {
            ...
            // Панель инструментов
            toolStrip = new ToolStrip { Dock = DockStyle.Top, ImageScalingSize = new Size(24, 24) };
            ...
            for (int i = 0; i < toolNames.Length; i++)
            {
                ...
                toolButtons[i].Click += (s, e) => SetTool((Tool)toolIndex);
                toolStrip.Items.Add(toolButtons[i]);
            }
            this.Controls.Add(toolStrip);

            // Панель свойств
            propPanel = new Panel { Dock = DockStyle.Right, Width = 220, BackColor = Color.FromArgb(240, 240, 240) };
            ...
            btnForeColor.Click += (s, e) => {
                using (ColorDialog cd = new ColorDialog { Color = foreColor })
                {
            ...
            btnBackColor.Click += (s, e) => {
                using (ColorDialog cd = new ColorDialog { Color = backColor })
                {
            ...
            cmbStyle.SelectedIndexChanged += (s, e) =>
            {
                ...
                {
                    case 1: dashStyle = DashStyle.Dash; break;
                    case 2: dashStyle = DashStyle.Dot; break;
            ...
            this.MouseUp += (s, e) => { if (isDrawing) isDrawing = false; };

            canvasBox.MouseLeave += CanvasBox_MouseLeave; 
        ...
        {
            if (canvas == null) return; e.Graphics.DrawImage(canvas, 0, 0);

            if (isDrawing && currentTool != Tool.Pencil && currentTool != Tool.Eraser && currentTool != Tool.Fill)
            {
                using (Pen pen = new Pen(foreColor, thickness) { DashStyle = dashStyle })
                {
                    Rectangle rect = GetRectangle();
                    if (currentTool == Tool.Line) e.Graphics.DrawLine(pen, startPoint, currentPoint);
                    else if (currentTool == Tool.Rectangle) e.Graphics.DrawRectangle(pen, rect);
                    else if (currentTool == Tool.Ellipse) e.Graphics.DrawEllipse(pen, rect);
                }
            }
        }
        ...
        private void CanvasBox_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            isDrawing = true;
            startPoint = lastPoint = currentPoint = e.Location;
        ...
        private void CanvasBox_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (currentTool == Tool.Line || currentTool == Tool.Rectangle || currentTool == Tool.Ellipse)
            {
                ...
                {
                    Rectangle rect = GetRectangle();
                    if (currentTool == Tool.Line) g.DrawLine(pen, startPoint, currentPoint);
                    else if (currentTool == Tool.Rectangle) g.DrawRectangle(pen, rect);
                    else if (currentTool == Tool.Ellipse) g.DrawEllipse(pen, rect);
                }
                canvasBox.Invalidate();
            }
        }
        ...
        {
            if (isDrawing) { isDrawing = false; canvasBox.Capture = false; }
        }
        ...
        {
            if (canvasBox.ClientSize.Width <= 0 || canvasBox.ClientSize.Height <= 0) return;
            if (canvas != null) canvas.Dispose();

            canvas = new Bitmap(canvasBox.ClientSize.Width, canvasBox.ClientSize.Height);
            using (Graphics g = Graphics.FromImage(canvas)) g.Clear(backColor);
            canvasBox.Invalidate();
            undoStack.Clear();
            redoStack.Clear();
        }
        ...
        {
            using (OpenFileDialog ofd = new OpenFileDialog { Filter = "PNG|*.png|JPEG|*.jpg;*.jpeg|BMP|*.bmp", Title = "Открыть изображение" })
            {
        ...
        {
            using (SaveFileDialog sfd = new SaveFileDialog { Filter = "PNG|*.png|JPEG|*.jpg;*.jpeg|BMP|*.bmp", Title = "Сохранить изображение" })
            {
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        ImageFormat fmt;
                        if (ext == ".png") fmt = ImageFormat.Png;
    ...
    {
        public DoubleBufferedPictureBox() { this.DoubleBuffered = true; }
    }
